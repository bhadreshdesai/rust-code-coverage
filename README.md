...
`Coverage Gutters: Watch` is activated automatically using `coverage-gutters.watchOnActivate`
```shell
...
Note: Coverage Gutters: Watch works on codespaces if the coverage file is moved to `./coverage/lcov.info` For some reason when the file is under the `target` folder and it changes the file watcher does not trigger a change event 
